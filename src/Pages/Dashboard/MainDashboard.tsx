...
const MainDashboard: React.FC<MainDashboardProps> = ({ setSelectedTaskId }) => {
  ...
  return (
    <div className="w-full lg:px-5 py-10 flex flex-col justify-center items-center lg:items-start gap-5">
      <div className="w-full flex justify-between items-start">
        <div
          className="px-[25px] py-[5px] rounded-[8px] bg-[#4A90E2] cursor-pointer flex flex-row-reverse justify-center items-center gap-2 transition-all duration-300 ease-in-out hover:scale-105"
      ...
      <div className="hidden lg:flex w-full flex-col lg:flex-row gap-10 mt-10">
        {statuses.map(status => {
      ...
      <div className="lg:hidden mt-10 w-full flex flex-col gap-5">
        <div className="relative w-full">
          <div
            className="absolute right-0 w-[200px] h-[40px] flex flex-row-reverse justify-between items-center px-3 border border-[#B0B8C1] rounded-md cursor-pointer"
            onClick={() => setIsDropdownOpen(prev => !prev)}
          >
            <p className="text-[#2C3E50]">
              {selectedStatus === "pending" ? "در انتظار" : selectedStatus === "in-progress" ? "در حال انجام" : "تکمیل شده"}
            </p>
            <i className={`bi bi-chevron-down text-[#2C3E50] transition-transform duration-300 ${isDropdownOpen ? "rotate-180" : "rotate-0"}`}></i>
          </div>
          {isDropdownOpen && (
            <div className="absolute top-[40px] right-0 w-[200px] bg-white border border-[#B0B8C1] rounded-md shadow-md flex flex-col justify-start items-end z-50">
              {statuses.map((status) => (
                <p
                  key={status}
                  onClick={() => { setSelectedStatus(status); setIsDropdownOpen(false); }}
                  className="w-full text-[#2C3E50] hover:bg-[#F0F0F0] px-3 py-1 text-right cursor-pointer"
                >
                  {status === "pending" ? "در انتظار" : status === "in-progress" ? "در حال انجام" : "تکمیل شده"}
                </p>
              ))}
        ...
        <ul className="w-full flex flex-col justify-center items-center gap-4 mt-[50px]">
          {userTasks.filter(task => task.status === selectedStatus).length > 0 ? (
            userTasks.filter(task => task.status === selectedStatus).map(task => (
              <li key={task.taskId} className="py-4 px-3 w-full flex flex-col justify-start items-end border border-[#B0B8C1] rounded-md gap-3 transition-all duration-300 ease-in-out hover:shadow-md hover:shadow-[#E4E8EC]">
                <div><p className="text-[#2C3E50]">{task.title}</p></div>
                <div className="w-full flex justify-between items-start">
                  <div className="text-[13px]">
                    {task.priority==="high" && <p className="text-[#FFFFFF] bg-red-600 px-2 rounded-lg text-center">بالا</p>}
      ...
      {toast.message && (
        <div dir="rtl" className={`fixed top-10 left-1/2 -translate-x-1/2 transform z-[9999] text-white px-4 lg:px-6 py-3 rounded-2xl shadow-lg text-lg transition-all duration-300 ${toast.type === "success" ? "bg-green-500/80" : "bg-red-500/80"}`}>
          {toast.message}
        </div>
      )}
    </div>
